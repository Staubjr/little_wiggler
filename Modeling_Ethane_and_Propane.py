...
class simulation():
    ...
    def initialize_non_bonding_pairs(self):
        ...
        for j in range(0, len(all_atoms)):
            for i in range(0, j):
                atom1 = all_atoms[i]
                atom2 = all_atoms[j]
                ...
                if make_interaction == True:
                    ...
                    electrostatic_pair = electrostatic(atom1, atom2)
    ...
    def graph_energy_conservation(self, name, save_image = False):
        
        os.chdir(Code_config_path)
        os.chdir(Simulation_config_path)
        
        fig, ax = plt.subplots( 1, 2, sharex = True, sharey = True, figsize = (16, 9))
        ax[0].plot(self.times, self.total_energies, label = 'Total Energy', color = 'r', linestyle = '--')
        ax[0].plot(self.times, self.works, label = 'Total_Work', color = 'b', linestyle = '--')
        ax[0].plot(self.times, self.energy_plus_total_work, label = 'Total Energy + Work', color = '#4F0066')
        ax[1].plot(self.times, self.potential_energies, label = 'Potential Energy', color = 'm')
        ax[1].plot(self.times, self.kinetic_energies, label = 'Kinetic Energy', color = 'c')
        ...
        ax[0].set_xlabel('Time (~$10^{-14}$s)')        
        ax[1].set_xlabel('Time (~$10^{-14}$s)')
        
        ax[0].set_ylabel('Energy (eV)')
        ax[0].set_title('Work and Energy')
        ax[1].set_title('Potential and Kinetic Energy')
        fig.show()
        plt.show()

        if save_image == True:
            fig.savefig(name, type = '.svg' )
    ...
    def hydrate(self, number_of_water):
        ...
        while waters_added < number_of_water:
            ...
            O1 = atom( atom_number = 3, element = 'oxygen'  , element_type = 'OT', x0 = random_position[0], y0 = random_position[1], z0 = random_position[2])
            H1 = atom( atom_number = 1, element = 'hydrogen', element_type = 'HT')
            H2 = atom( atom_number = 1, element = 'hydrogen', element_type = 'HT')
            ...
            for specific_molecule in self.molecules:
                for specific_atom in specific_molecule.atoms:
                    all_atoms.append(specific_atom)

            for other_atom in all_atoms:
                
                lennard_jones(H1, other_atom)
                electrostatic(H1, other_atom)
                
                lennard_jones(H2, other_atom)
                electrostatic(H2, other_atom)
                
                lennard_jones(O1, other_atom)
                electrostatic(O1, other_atom)
            ...
            while update_water == True:

                if abs(final_potential - initial_potential) < 1.25 * initial_potential:
...
class molecule():
    ...
    dampening_coef = 0.0  # I typically use 1.0 for 'good' results
...
class bond:
    ...
    def __init__(self, atom1, atom2, counted = False, number_of_bonds = 1):
        ...
        if self.atom1.potential_bonds < len(self.atom1.bonds):
            sys.stderr('A {} is going to be bonded to {} other atoms.'.format(str(self.atom1.element), len(self.atom1.bonds + 1)))

        if self.atom2.potential_bonds < len(self.atom2.bonds):
            sys.stderr('A {} is going to be bonded to other atoms.'.format(str(self.atom2.element), len(self.atom2.bonds + 1)))
            
        self.counted = counted
        self.number_of_bonds = number_of_bonds
        
        atom1.bonds.append(self)
        atom1.atoms_bonded_to.append(atom2)
        atom2.bonds.append(self)
        atom2.atoms_bonded_to.append(atom1)
    ...
    def bond_potential(self):
        ...
        potential =  self.k_bond * (distance - self.E0_bond)**2
...
def main():
    ...
    my_MD_simulation = simulation(squishy_sphere = True, radius = 10000.0, stiffness = 1E3)
...
################## Ethane ##################################
    ...
    # c_1 = atom(element = 'carbon', element_type = 'CC33A', atom_number = 1, y0 =  l0/2)
    # c_2 = atom(element = 'carbon', element_type = 'CC33A', atom_number = 2, y0 =  l0 * s20, x0 =  l0 * c20)

    # h_1 = atom(atom_number = 3, element = 'hydrogen', element_type = 'HCA3A', x0 = c_1.pos[0] - h_bond - 0.01, y0 = c_1.pos[1] + h_bond, z0 = .5)
    # h_2 = atom(atom_number = 4, element = 'hydrogen', element_type = 'HCA3A', x0 = c_1.pos[0] + h_bond, y0 = c_1.pos[1] + h_bond, z0 = -0.5)
    # h_3 = atom(atom_number = 5, element = 'hydrogen', element_type = 'HCA3A', x0 = c_1.pos[0] + h_bond, y0 = c_1.pos[1] - h_bond, z0 = -0.5)    
    
    # h_4 = atom(atom_number = 6, element = 'hydrogen', element_type = 'HCA3A', x0 = c_2.pos[0] + h_bond, y0 = c_2.pos[1] + h_bond)
    # h_5 = atom(atom_number = 7, element = 'hydrogen', element_type = 'HCA3A', x0 = c_2.pos[0] + l1, y0 = c_2.pos[1])
    # h_6 = atom(atom_number = 7, element = 'hydrogen', element_type = 'HCA3A', x0 = c_2.pos[0] + l1, y0 = c_2.pos[1], z0 = -1.)    
    
################### Propane ##############################

    c_1 = atom(element = 'carbon', element_type = 'CC33A', atom_number = 1, x0 =  0)
    c_2 = atom(element = 'carbon', element_type = 'CC32A', atom_number = 2, x0 =  1, y0 = 0.25)
    c_3 = atom(element = 'carbon', element_type = 'CC33A', atom_number = 3, x0 =  l0 * 2, y0 = -0.3)
    
    h_1 = atom(atom_number = 3, element = 'hydrogen', element_type = 'HCA3A', x0 = c_1.pos[0] - h_bond - 0.01, y0 = c_1.pos[1] + h_bond, z0 = .5)
    h_2 = atom(atom_number = 4, element = 'hydrogen', element_type = 'HCA3A', x0 = c_1.pos[0] - h_bond, y0 = c_1.pos[1] + h_bond, z0 = -0.5)
    h_3 = atom(atom_number = 5, element = 'hydrogen', element_type = 'HCA3A', x0 = c_1.pos[0] - h_bond, y0 = c_1.pos[1] - h_bond, z0 = -0.5)    
    
    h_4 = atom(atom_number = 6, element = 'hydrogen', element_type = 'HCA2A', x0 = c_2.pos[0] , y0 = c_2.pos[1] + h_bond, z0 = 0.1)
    h_5 = atom(atom_number = 7, element = 'hydrogen', element_type = 'HCA2A', x0 = c_2.pos[0] , y0 = c_2.pos[1] - h_bond, z0 = 0.3)

    h_6 = atom(atom_number = 12, element = 'hydrogen', element_type = 'HCA3A', x0 = c_3.pos[0] + h_bond, y0 = c_3.pos[1] - h_bond, z0 = -0.1)
    h_7 = atom(atom_number = 13, element = 'hydrogen', element_type = 'HCA3A', x0 = c_3.pos[0] + h_bond, y0 = c_3.pos[1] + h_bond, z0 = 0.35)
    h_8 = atom(atom_number = 14, element = 'hydrogen', element_type = 'HCA3A', x0 = c_3.pos[0] + h_bond, y0 = c_3.pos[1] - h_bond, z0 = -0.5)
...
######################################################################
    ...
    my_MD_simulation.molecules.append(propane)
    ...
    # ethane.bond_atoms(h_1 , c_1, 1)
    # ethane.bond_atoms(h_2 , c_1, 1)
    # ethane.bond_atoms(h_3 , c_1, 1)    
    # ethane.bond_atoms(h_4 , c_2, 1)
    # ethane.bond_atoms(h_5 , c_2, 1)
    # ethane.bond_atoms(h_6 , c_2, 1)
    
    # ethane.bond_atoms(c_1, c_2, 1)

    propane.add_atom(h_1)
    propane.add_atom(h_2) 
    propane.add_atom(h_3)
    propane.add_atom(h_4)
    propane.add_atom(h_5)
    propane.add_atom(h_6)
    propane.add_atom(h_7)
    propane.add_atom(h_8)
    propane.add_atom(c_1)
    propane.add_atom(c_2)
    propane.add_atom(c_3)
    
    propane.bond_atoms(h_1 , c_1, 1)
    propane.bond_atoms(h_2 , c_1, 1)
    propane.bond_atoms(h_3 , c_1, 1)    
    propane.bond_atoms(h_4 , c_2, 1)
    propane.bond_atoms(h_5 , c_2, 1)
    propane.bond_atoms(h_6 , c_3, 1)
    propane.bond_atoms(h_7 , c_3, 1)
    propane.bond_atoms(h_8 , c_3, 1)
    
    propane.bond_atoms(c_1, c_2, 1)
    propane.bond_atoms(c_2, c_3, 1)
...
#############################################################################################################################
    ...
    # for bangle in propane.bond_angles:
